...
<body class="min-h-screen w-screen overflow-x-hidden bg-cover bg-center bg-no-repeat bg-fixed" style="background-image: url(<?= assets('website/images/default-bg.png') ?>)">
  <main id="app" class="my-10 md:my-20">
    ...
    <section class="mt-10 max-w-screen-2xl mx-auto px-10 md:px-20 2xl:px-10">
      <div class="w-full rounded-md bg-white p-6 shadow-md">
        ...
        <table class="mt-14 block xl:table overflow-x-auto w-full max-w-full text-left font-normal text-[#4A4251] text-sm">
          ...
          <tbody>
            <tr v-for="member in teamMembers" v-show="member.isVisible" class="even:bg-white odd:bg-gray-100">
              ...
              <td class="p-3 flex space-x-1 items-center">
                <a :href="`${baseURL}admin/teamMembers/update/${member.id}`">
            ...
            <tr v-show="teamMembers.every(member => !member.isVisible)" class="bg-gray-100">
    ...
    <?php $this->load->view('admin/team-members/approve-confirmation-modal', $this->data) ?>
  </main>
    ...
    Vue.createApp({
      ...
      methods: {
        async deleteRecord () {
          ...
          const index = this.teamMembers.findIndex(member => Number(member.id) === Number(response.memberId))
          this.teamMembers.splice(index, 1)

          closeDeleteConfirmationModal()
          showNotify('Membro excluído com sucesso')
        },
        openDeleteConfirmationModal (memberId) {
          const action = `${this.baseURL}admin/teamMembers/delete/${memberId}`
        ...
        async approveRecord () {
          ...
          this.teamMembers[index].status = 'approved'
          this.teamMembers[index].subscription_number = response.subscriptionNumber
          ...
          showNotify('Membro aprovado com sucesso')
        },
        openApproveConfirmationModal (memberId) {
          const member = this.teamMembers.find(member => Number(member.id) === Number(memberId))
          this.currentMemberToApprove = member

          const action = `${this.baseURL}admin/teamMembers/approve`
        ...
        downloadCSV () {
          ...
          const visibleMembers = this.teamMembers.filter(member => member.isVisible)

          if (!visibleMembers.length) {
            return showNotify('Nenhum membro para baixar')
          }
          ...
          visibleMembers.forEach(member => {
            rows.push([
              ...
              this.getTranslatedMemberType(member.type),
        ...
        getMemberStatusDescription (memberId) {
          const member = this.teamMembers.find(member => member.id === memberId)
        ...
        runSearch () {
          ...
          const propsToCompare = ['team_name', 'name', 'cpf', 'rg', 'birth_date', 'type', 'status']
      ...
      mounted () {
        for (member in this.teamMembers) {
          this.teamMembers[member].isVisible = true
        }
